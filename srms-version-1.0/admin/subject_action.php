...
if(isset($_POST["hidden_action"]))
{
	if($_POST["hidden_action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-primary btn-sm status_button" data-id="'.$row["class_id"].'" data-status="'.$row["subject_status"].'">Enable</button>';
			}
			else
			{
				$status = '<button type="button" name="status_button" class="btn btn-danger btn-sm status_button" data-id="'.$row["class_id"].'" data-status="'.$row["subject_status"].'">Disable</button>';
			}
			$sub_array[] = $status;
			$sub_array[] = '
			<div align="center">
	...
	if($_POST["hidden_action"] == 'Add')
	{
		...
		{
			$error = '<div class="alert alert-danger">Subject Name Already Exists in <b>'.$object->Get_class_name($_POST["class_id"]).'</b> Class</div>';
		}
		else
		{
			$data = array(
			...
			$success = '<div class="alert alert-success">Subject Added in <b>'.$object->Get_class_name($_POST["class_id"]).'</b> Class</div>';
		}
